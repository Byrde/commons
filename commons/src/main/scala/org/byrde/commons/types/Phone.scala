...
import Phone.{ Area, Country, Exchange, Ext }
...
object Phone {
  ...
  private val NorthAmericanNumber: String => Boolean = phone => phone.length == 11 && phone.startsWith("1")

  private val AustralianNumber: String => Boolean = phone => phone.length == 11 && phone.startsWith("61")
  ...
  private[commons] def toNorthAmericanPhone(phone: String): Either[PhoneValidationError, Phone] = {
    def extractCountryCode(phone: String): String = phone.charAt(0).toString

    def extractAreaCode(phone: String): String = phone.substring(1, 4)

    def extractExchangeCode(phone: String): String = phone.substring(4, 7)

    def extractExtensionCode(phone: String): String = phone.substring(7, 11)
  ...
  private[commons] def toAustralianPhone(phone: String): Either[PhoneValidationError, Phone] = {
    def extractCountryCode(phone: String): String = phone.substring(0, 2)

    def extractAreaCode(phone: String): String = phone.charAt(2).toString

    def extractExchangeCode(phone: String): String = phone.substring(3, 7)
  ...
  private def normalizePhone(phone: String): String = phone.trim.replaceAll("[^\\d]", "")
}

case class Phone(country: Country, area: Area, exchange: Exchange, extension: Ext) {
  def unformatted: String = s"$country$area$exchange$extension"

  override def toString: String = s"+$country-$area-$exchange-$extension"
}
