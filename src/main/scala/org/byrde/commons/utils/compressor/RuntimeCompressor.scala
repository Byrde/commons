...
case class RuntimeCompressor(_htmlConfig: Option[HtmlCompressorConfig] = None,
  ...
  private def commonCompress(is: InputStream, target: File) = {
    ...
    if (ext == "html") {
      Future(htmlConfig.compressor.compress(IOUtils.toString(is, "UTF-8")))
        .map { html =>
          ow.write(html)
        }
    } else if (ext == "css") {
      Future(
        CssCompressorConfig().compressor.compress(
          IOUtils.toString(is, "UTF-8"))).map { html =>
        ow.write(html)
      }
    } else if (ext == "js") {
      Future(jsConfig.compressor.compress(IOUtils.toString(is, "UTF-8"))).map {
        html =>
          ow.write(html)
      }
    } else {
      org.byrde.commons.utils
