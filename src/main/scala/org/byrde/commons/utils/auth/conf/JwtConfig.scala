...
import inc.discoverly.commons.models.utils.auth.definition.Admin
...
object JwtConfig{
	...
	def apply(_token: String, _signature: String, _encryption: String, _claims: String, config: Configuration): JwtConfig = {
		val token =
			config.get[String](_token)
		val signature =
			config.get[String](_signature)
		val encryption =
			config.get[String](_encryption)
		val claims =
			config
  ...
  saltOpt: Option[String] = None){
	lazy val signature: String = {
		saltOpt.fold(secret) { salt =>
			Base64.encodeBase64URLSafeString(secret.getBytes("UTF-8") ++ salt.getBytes("UTF-8"))
		}
	}
}
