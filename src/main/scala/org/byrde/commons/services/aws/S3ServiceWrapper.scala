...
case class S3ServiceWrapper(
  ...
  def storeFile(file: File, fileName: Option[String] = None): Future[ServiceResponse[String]] = {
    if(!client.doesBucketExist(s3Config.bucketName))
      client.createBucket(s3Config.bucketName)

    Future(client.putObject(s3Config.bucketName, fileName.getOrElse(file.getName), file)).map { _ =>
      E0200.withMessage("File uploaded successfully")
    }
  }

  def retrieveFile(fileName: String): Future[S3ObjectInputStream] =
    Future(client.getObject(s3Config.bucketName, fileName)).map(_.getObjectContent)
