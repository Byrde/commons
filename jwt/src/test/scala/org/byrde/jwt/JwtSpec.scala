...
class JwtSpec extends AnyFlatSpec with Matchers {
  ...
  it should "succeed if the subject is provided and matches" in {
    implicit val config: JwtConfig = JwtConfig("test", 5000L, JwtAlgorithm.HS256, "test")
    new JwtClient().validateJwt[Claims](
      new JwtClient().issueJwt(claims, subject = Some("test")),
      subject = Some("test"),
  ...
  "Jwt.decode" should "fail on expired token" in {
    ...
    val validationError =
      new JwtClient().validateJwt[Claims](new JwtClient().issueJwt(claims)).swap.toOption.get.asInstanceOf[Expired]
  ...
  it should "fail on invalid token" in {
    ...
    val validationError =
      new JwtClient()
        .validateJwt[Claims](new JwtClient()(config.copy(secret = "test1")).issueJwt(claims))
        .swap
        .toOption
        .get
  ...
  it should "fail if the issuer does not match" in {
    ...
    val validationError =
      new JwtClient()
        .validateJwt[Claims](new JwtClient()(config.copy(issuer = "test1")).issueJwt(claims))
        .swap
        .toOption
        .get
        .asInstanceOf[Invalid]
  ...
  it should "fail if the subject is provided but does not match" in {
    ...
    val validationError =
      new JwtClient()
        .validateJwt[Claims](new JwtClient().issueJwt(claims, subject = Some("test")), subject = Some("test1"))
        .swap
        .toOption
        .get
        .asInstanceOf[Invalid]
