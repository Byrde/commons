...
class JwtSpec extends AnyFlatSpec with Matchers {
  ...
  "Jwt.encode" should "encode correctly encode/decode" in {
    ...
    JwtParser.validateJwt[Claims](JwtParser.issueJwt(claims)) shouldBe Right(claims)
  }
  
  it should "succeed if the subject is provided and matches" in {
    implicit val config: JwtConfig = JwtConfig("test", 5000L, JwtAlgorithm.HS256, "test")
    JwtParser.validateJwt[Claims](JwtParser.issueJwt(claims, subject = Some("test")), subject = Some("test")) shouldBe Right(claims)
  }

  "Jwt.decode" should "fail on expired token" in {
    implicit val config: JwtConfig = JwtConfig("test", -1L, JwtAlgorithm.HS256, "test")
    val validationError = JwtParser.validateJwt[Claims](JwtParser.issueJwt(claims)).swap.toOption.get.asInstanceOf[Expired]
    validationError shouldBe Expired(validationError.message)
  }

  it should "fail on invalid token" in {
    implicit val config: JwtConfig = JwtConfig("test", 5000L, JwtAlgorithm.HS256, "test")
    val validationError = JwtParser.validateJwt[Claims](JwtParser.issueJwt(claims)(config.copy(secret = "test1"), deriveEncoder[Claims])).swap.toOption.get.asInstanceOf[Invalid]
    validationError shouldBe Invalid(validationError.message)
  }
  
  it should "fail if the issuer does not match" in {
    implicit val config: JwtConfig = JwtConfig("test", 5000L, JwtAlgorithm.HS256, "test")
    val validationError = JwtParser.validateJwt[Claims](JwtParser.issueJwt(claims)(config.copy(issuer = "test1"), deriveEncoder[Claims])).swap.toOption.get.asInstanceOf[Invalid]
    validationError shouldBe Invalid(validationError.message)
  }
  
  it should "fail if the subject is provided but does not match" in {
    implicit val config: JwtConfig = JwtConfig("test", 5000L, JwtAlgorithm.HS256, "test")
    val validationError = JwtParser.validateJwt[Claims](JwtParser.issueJwt(claims, subject = Some("test")), subject = Some("test1")).swap.toOption.get.asInstanceOf[Invalid]
    validationError shouldBe Invalid(validationError.message)
  }
}
