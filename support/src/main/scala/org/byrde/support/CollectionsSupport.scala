...
trait CollectionsSupport {
  implicit class IterableOnceExtensions[A](val col: IterableOnce[A]) {
    /**
      ...
      @param initial initial element to start the accumulation on
      @param accumulate accumulation function
    */
    def foldWhile[A1 >: A](
      predicate: A => Boolean,
      initial: A1
    )(accumulate: (A1, A1) => A1): A1 =
      foldLeftWhile[A1](predicate, initial)(accumulate)
    ...
    */
    ...
    /**
      Fold while element satisfies the predicate. Stops on first unsatisfied predicate.
      @param predicate check on the next applied element to see whether we should continue to iterate
      @param initial initial element to start the accumulation on
      @param accumulate accumulation function
     */
