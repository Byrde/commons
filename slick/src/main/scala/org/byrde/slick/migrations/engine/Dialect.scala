...
import AstHelpers._
import org.byrde.slick.migrations.engine.TableMigration.TableMigrationData
...
class Dialect[-P <: JdbcProfile] extends AstHelpers {
  def quoteIdentifier(id: String): String = {
    val s =
      new StringBuilder(id.length + 4) append '"'
  ...
  protected def quotedColumnNames(ns: Seq[FieldSymbol]): Seq[String] =
    ns.map(fs => quoteIdentifier(fs.name))

  def columnType(ci: ColumnInfo): String =
    ci.sqlType

  def autoInc(ci: ColumnInfo): String =
    if(ci.autoInc) " AUTOINCREMENT" else ""
  ...
  def notNull(ci: ColumnInfo): String =
    if (ci.notNull) " NOT NULL" else ""

  def columnSql(ci: ColumnInfo, newTable: Boolean = true): String = {
    def name =
      quoteIdentifier(ci.name)

    def typ =
      columnType(ci)

    def default =
      ci.default.map(" DEFAULT " + _).getOrElse("")
  ...
  def createTable(table: TableInfo, columns: Seq[ColumnInfo]): String =
    s"""create table ${quoteTableName(table)} (
      | ${columns map { columnSql(_) } mkString ", "}
      |)""".stripMargin
  ...
  def renameIndex(old: IndexInfo, newName: String): Seq[String] =
    List(
  ...
  def dropColumn(table: TableInfo, column: String): String =
    s"""alter table ${quoteTableName(table)}
  ...
  def migrateTable[E <: P](table: TableInfo, tmd: TableMigrationData): Seq[String] = {
    ...
    val createColumns =
      if(tmd.tableCreate) {
        ...
        tmd.tableRename.map{ renameTable(table, _) }.toList ++ tmd.columnsCreate.map{ addColumn(table, _) }
...
object Dialect {
  class DerbyDialect extends Dialect[DerbyProfile] {
    ...
    override def alterColumnType(table: TableInfo,
                                 column: ColumnInfo): List[String] = {
      val tmpColumnName =
        "tempcolumn" + (math.random * 1000000).toInt

      val tmpColumn =
        column.copy(name = tmpColumnName)

      List(
        addColumn(table, tmpColumn),
        s"update ${quoteTableName(table)} set ${quoteIdentifier(tmpColumnName)} = ${quoteIdentifier(
          column.name)}",
    ...
    override def renameColumn(table: TableInfo,
                              from: ColumnInfo,
                              to: String) =
      s"rename column ${quoteTableName(table)}.${quoteIdentifier(from.name)} to ${quoteIdentifier(to)}"

    override def alterColumnNullability(table: TableInfo,
  ...
  class SQLiteDialect
      extends Dialect[SQLiteProfile]
      with SimulatedRenameIndex[SQLiteProfile] {
    override def columnType(ci: ColumnInfo): String =
  ...
  class HsqldbDialect extends Dialect[HsqldbProfile] {
    ...
    override def notNull(ci: ColumnInfo): String =
      if (ci.notNull && !ci.isPk) " NOT NULL" else ""
  }

  class MySQLDialect
      extends Dialect[MySQLProfile]
      with SimulatedRenameIndex[MySQLProfile] {
    override def autoInc(ci: ColumnInfo): String =
    ...
    override def dropIndex(index: IndexInfo) =
      s"drop index ${quoteIdentifier(index.name)} on ${quoteTableName(
    ...
    override def createPrimaryKey(table: TableInfo,
                                  name: String,
                                  columns: Seq[FieldSymbol]): String =
      s"""alter table ${quoteTableName(table)}
    ...
    def apply(driver: JdbcProfile): Dialect[_ <: JdbcProfile] = driver match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          "Slick error : Unknown or unsupported jdbc driver found.")
    }
  }
}
