...
import org.byrde.slick.SlickHasPrivilege
import org.byrde.slick.SlickRole.Master
import org.byrde.slick.conf.{SlickMigrationEngineConfig, SlickProfile}
...
class SlickMigrationEngine(migrations: Seq[SlickNamedMigration])(implicit val ec: ExecutionContext, migrationEngineConfig: SlickMigrationEngineConfig = SlickMigrationEngineConfig(1.second, 1.second)) {
  ...
  private def createTable(retry: Int, limit: Int)(implicit ev: Master SlickHasPrivilege profile.api.Effect.All): Future[Unit] =
    run {
      MTable
        .getTables
        .flatMap { tables =>
  ...
  private def runMigration(migration: SlickNamedMigration)(implicit ev: Master SlickHasPrivilege profile.api.Effect.All): Future[Unit] =
    runMigration(UUID.randomUUID(), migration)

  private def runMigration(id: UUID, migration: SlickNamedMigration)(implicit ev: Master SlickHasPrivilege profile.api.Effect.All): Future[Unit] =
    for {
  ...
  private def claimMigration(migration: SlickNamedMigration, id: UUID)(implicit ev: Master SlickHasPrivilege profile.api.Effect.All): Future[Unit] =
    run(MigrationTable += MigrationRow(migration.name, id, "Requested", System.currentTimeMillis())).map(_ => ())

  private def checkClaimedOrCompleted(id: UUID, migration: SlickNamedMigration)(implicit ev: Master SlickHasPrivilege profile.api.Effect.All): Future[ClaimStatus] =
    checkClaimedOrCompleted(id, migration.name, migration)

  private def checkClaimedOrCompleted(id: UUID, name: String, migration: SlickNamedMigration)(implicit ev: Master SlickHasPrivilege profile.api.Effect.All): Future[ClaimStatus] =
  ...
  private def markAsCompleted(migration: SlickNamedMigration, id: UUID)(implicit ev: Master SlickHasPrivilege profile.api.Effect.All): Future[Unit] =
    run {
      MigrationTable
