...
private [engine] object AstHelpers {
  /**
 ...
 */
private [engine] trait AstHelpers {
  import AstHelpers._
    ...
    column.tpe match {
      case driver.JdbcType(ti, isOpt) =>
        ...
        column.options.foldLeft(initial) {
          case (ci, SColumnOption.SqlType(s)) =>
            ci.copy(sqlType = s)

          case (ci, SColumnOption.NotNull) =>
            ci.copy(notNull = true)

          case (ci, SColumnOption.Nullable) =>
            ci.copy(notNull = false)

          case (ci, AColumnOption.AutoInc) =>
            ci.copy(autoInc = true)

          case (ci, AColumnOption.PrimaryKey) =>
            ci.copy(isPk = true)

          case (ci, RColumnOption.Default(v)) =>
            ci.copy(default = Some(ti.valueToSQLLiteral(v)))
