...
object ServerConfig {
  def apply(config: Config): ServerConfig = apply("name", "version", "interface", "port", "cors", config)
  ...
  def apply(
    ...
    val _version = config.getString(version)

    val _interface = config.getString(interface)

    val _port = config.getInt(port)

    val _cors = CorsConfig(config.getConfig(cors))
