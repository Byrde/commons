...
trait CorsSupport {
  private def origins(config: CorsConfig): Seq[RawHeader] = config.origins.map {
    RawHeader("Access-Control-Allow-Origin", _)
  }
  
  private def allowedMethods(config: CorsConfig): Seq[RawHeader] = config.methods.map {
    RawHeader("Access-Control-Allow-Methods", _)
  }
  
  private def allowHeaders(config: CorsConfig): Seq[RawHeader] = config.headers.map {
    RawHeader("Access-Control-Allow-Headers", _)
  }
  
  private def exposeHeaders(config: CorsConfig): Seq[RawHeader] = config.headers.map {
