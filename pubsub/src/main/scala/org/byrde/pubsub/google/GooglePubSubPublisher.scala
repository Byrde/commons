...
/** Google Cloud Pub/Sub implementation of MessagePublisher.
  *
  * @param config
...
class GooglePubSubPublisher(
  ...
  override def publish[T](envelope: Envelope[T])(implicit encoder: Encoder[T]): Future[Either[PubSubError, Unit]] =
    Try {
      publishInternal(config.credentials, config.project, envelope, config.hostOpt)
    } match {
      case Success(result) => result.map(Right(_))
      case Failure(ex) =>
        Future.successful(Left(PubSubError.PublishError(s"Failed to publish to ${envelope.topic}", ex)))
    }

  @tailrec
  private def publishInternal[T](
    ...
    _publishers.get(env.topic) match {
      ...
      case None =>
        ...
        publishInternal(credentials, project, env, hostOpt)
    }
