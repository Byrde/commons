...
class FluentLogger(name: String) extends Logger {
  private case class MessageLog(value: String) extends Log {
    override def asMap: Map[String, String] = Map("message" -> value)
  }
  ...
  override def logDebug(msg: String): Unit = logger.log("debug", "message", msg)

  override def logDebug(msg: String, extras: Log*): Unit =
    logger.log(
      "debug",
      (MessageLog(msg) ++ extras.foldLeft(Log.empty)(_ ++ _)).asMap.asInstanceOf[Map[String, AnyRef]].asJava,
    )
  ...
  override def logWarning(msg: String, cause: Throwable, extras: Log*): Unit =
    logger.log(
      "warning",
      (MessageLog(msg) ++ ThrowableLog(cause) ++ extras
        .foldLeft(Log.empty)(_ ++ _)).asMap.asInstanceOf[Map[String, AnyRef]].asJava,
    )
